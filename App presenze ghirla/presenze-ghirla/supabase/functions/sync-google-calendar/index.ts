...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const dateStr of targetDates) {
      ...
      if (searchRes.ok) {
        ...
        if (searchData.items && searchData.items.length > 0) {
          for (const item of searchData.items) {
            // Elimina vecchi eventi duplicati o se per quel giorno non ci sono né presenze né ricorrenze speciali
            if ((totalPresencesCount === 0 && occasions.length === 0) || item.id !== eventId) {
      ...
      if (totalPresencesCount > 0 || occasions.length > 0) {
        const parts = dateStr.split('-');
        const formattedDate = parts.length === 3 ? `${parts[2]}/${parts[1]}/${parts[0]}` : dateStr;

        let eventSummary = '';
        if (occasions.length > 0 && totalPresencesCount > 0) {
          const occPrefix = occasions.map(o => o.title).join(' & ');
          eventSummary = `${occPrefix} | Ghirla: 🍝 ${lunchPeople.length} | 🍷 ${dinnerPeople.length} | 🛏️ ${overnightPeople.length}`;
        } else if (occasions.length > 0 && totalPresencesCount === 0) {
          const occPrefix = occasions.map(o => o.title).join(' & ');
          eventSummary = `${occPrefix} - Casa Ghirla`;
        } else {
          eventSummary = `Presenze Ghirla: 🍝 ${lunchPeople.length} | 🍷 ${dinnerPeople.length} | 🛏️ ${overnightPeople.length}`;
        }

        const descriptionLines = [
          `🏠 CASA GHIRLA - ${formattedDate}`,
        ...
        const eventBody = {
          ...
          reminders: {
            useDefault: false,
            overrides: [
              { method: 'popup', minutes: 0 }, // Notifica alle ore 09:00
              { method: 'popup', minutes: 60 } // Promemoria alle ore 08:00
            ]
          }
        };
        ...
        // Se 0 presenze e nessuna ricorrenza speciale, cancella l'evento
